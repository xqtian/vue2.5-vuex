<template>
  <section>
      ...
      <div v-if="fatherComponent == 'home'">
          ...
          <router-link to="item" @click="start" class="start button_style"></router-link>
      </div>
              ...
              <div class="item_list_container" v-if="itemDetail.length > 0">
                  ...
                  <ul>
                      <li v-for="(item, index) in itemDetail[itemNum-1].topic_answer" @click="choosed(index, item.topic_answer_id)" class="item_list">
                          <span class="option_style" v-bind:class="{'has_choosed':chooseNum == index}">
                              {{chooseType(index)}}
                          </span>
  ...
  methods: {
      ...
      nextItem(){
          if(this.chooseNum != null){
              ...
              this.itemNum++; // 题目编号加一
              this.answerid.push(this.chooseId); // 保存答案
          }else{
              alert('您还没有选择答案哦')
          }
      },
